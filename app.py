...
st.set_page_config(page_title="Query With Your Own Data ",page_icon=":scroll:",layout="wide")
...
def main():
    ...
    if selected == 'Chat':  
        st.file_uploader("Upload question as a image if you want !",type="png")

        user_question = st.text_input("Ask a question about your document  :scroll:",placeholder="Write Something...")
    ...
    if selected == 'Upload File':
        ...
        pdf = st.file_uploader("Upload Your Data Here :scroll:",type="pdf", accept_multiple_files=True)
        if st.button("Upload"):
            with st.spinner("Processing"):
                ...
                # create conversation 
                st.session_state.conversation = get_conversation_chain(user_question, vectorstore)

                st.write("Succesfully Upload,Now Go To Chat And Query Your Data")
